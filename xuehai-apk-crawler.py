...
dic={}
def enc(s):
    t=map(int,bin(date.today().day-1+32)[2:].rjust(len(s),'0'))
...
def extr(h):
    return re.sub('\\s+',' ',re.sub('[\\n\\r]+','',re.sub('<.*?>','',re.sub('<style.*?</style>','',re.sub('<script.*?</script>','',re.sub('<head.*?</head>','',h,flags=re.S),flags=re.S),flags=re.S),flags=re.S),flags=re.S)).replace('&quot;','"')
    ...
    with urllib2.urlopen(base+url) as r:
        ...
        try:
            ...
            def chk(x):
                ...
                if 'url' not in x['appVersions'][0]:print('Error in '+x['appId']);return False
            ...
            def req(i):
                ...
                if t['packageName']=='com.xh.mis':
                    t['appVersions'].pop()
                if t['packageName']=='com.xuehai.zhenzhen':
                    t['name']='镇镇'
                return t
            o=[req(i) for i in o]
            for x in o:
        ...
        except Exception as e:print('Invalid response!');print('%s: %s'%(e.__class__.__name__, str(e)))
except Exception as e:print('Invalid response!');print('%s: %s'%(e.__class__.__name__, str(e)))